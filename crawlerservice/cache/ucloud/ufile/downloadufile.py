...
class DownloadUFile(BaseUFile):
    ...
    def download_stream(self, bucket, key, isprivate=True, expires=config.get_default('expires'), content_range=None, header=None):
        """
        下载UFile文件并且按string 返回
        ...
        @return ret: 如果http状态码为[200, 204, 206]之一则返回string，否则如果服务器返回json信息则返回dict类型，键值对类型分别为string, unicode string类型，否则返回空的dict
