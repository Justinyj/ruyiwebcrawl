...
def process(url, batch_id, parameter, manager, other_batch_process_time, *args, **kwargs):
    ...
    for label, reg in process._regs.iteritems():
        ...
        if label == 'first_letter':
            first_letter = url
            data = {
                'Data':'{{\"url\": \"\", \"letter\": \"{}\"}}'.format(first_letter)
            }  # form data示例： Data:{"letter":"J","url":""} 
            ...
            if not sub_drug_list:           # 请求成功然而列表为空，说明这种药材没有价格数据
                return True

            for sub_drug in sub_drug_list:    # eg：一个刀豆list里根据不同规格和产地的刀豆会分为不同的sub_drug,拥有不同的MBSID
                print sub_drug['MSpec'].encode('utf-8')
                # print json.dumps(result_item, ensure_ascii=False)
                ...
                spec_info = sub_drug['MSpec'].split(' ')   # MSpec一般情况示例： 大统 东北
                productGrade =   spec_info[0]                                         
                if len(spec_info)==2:                      # MSpec特殊情况示例： 统 
                    productPlaceOfOrigin = spec_info[1]
                else:
                    productPlaceOfOrigin = ''
                price_item = json.loads(price_content)[u'Data']
                price_data = price_item[u'PriceChartData']  # price_data是一串字符串，需要再次loads后变为一个列表，每个列表代表一个药市，其中还嵌套着价格列表,价格里时间表示为时间戳等。
                                                            # 由于后续处理较复杂，决定将这部分留到清洗脚本中完成，爬虫层面避免过多的清洗操作。
                if price_data == '[]':   # 即使存在这种规格，也有可能没有价格历史
                    return True
                ...
                process._cache.post(url, json.dumps(result_item, ensure_ascii = False), refresh = True)
                # print json.dumps(result_item, ensure_ascii = False)
            return True
...
if __name__ == '__main__':
    process('1490', 'zhongyaocai-20160929','123','123',0)