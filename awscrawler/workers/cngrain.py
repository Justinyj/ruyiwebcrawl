...
def process(url, batch_id, parameter, manager, *args, **kwargs):
    ...
    for label, reg in process._regs.iteritems():
        ...
        if label == 'home':
            data = {
                #'__EVENTTARGET':,'ctl00$ContentPlaceHolder1$DG_FullLatestPrice$ctl24$ctl07', #最后两位表示页数，其值不能超过11
                '__EVENTARGUMENT':'',
            ...
            while 1:
                ...
                print (market_suffixes)
                if not market_suffixes:
                    print (content)
                    print('A ou!')

                market_suffixes_set = set(market_suffixes) #去重，但是对于市场名重复延续到下一页的情况无可奈何
                ...
                page_label = dom.xpath('//td[@colspan="10"]//span')[0]  #在十个页数里，只有当前页的标签是span，定位到当前页
                ...
                next_raw_js = next_sibling.xpath('./@href')[0]  # 形式为 :   "javascript:__doPostBack('ctl00$ContentPlaceHolder1$DG_FullLatestPrice$ctl24$ctl01','')"
                eventtarget = re.findall('\(\'(.*)\',',next_raw_js)[0]
                data['__EVENTTARGET'] = eventtarget    
                print (eventtarget)

                last = data['__ZIPSTATE'] #用来储存上一次的ZIPSTATE参数，如果新参数失败就更换旧的使用——因为实践过程中某页的ZIPSTATE有小概率对下一页失效
                data['__ZIPSTATE'] = (dom.xpath('//input[@name="__ZIPSTATE"]/@value'))[0]  #更新参数
                data['__EVENTVALIDATION'] = (dom.xpath('//input[@name="__EVENTVALIDATION"]/@value'))[0]  #更新参数
                # print (data['__EVENTVALIDATION'] )
                ...
                for _ in range(0,3):
                    try:
                        ...
                        if content == '':
                           print ('change ZIPSTATE')
                           data['__ZIPSTATE'] = last #使用上一次的，
                           continue
                        break
                    except Exception, e:
                        print (e)
            ...
            table_node = dom.xpath('//table[@class="data_table"]')[0]
            products_nodes = table_node.xpath('.//tr')[1:-1]
            ...
            for product_node in products_nodes:   #市场页会有相同产品在不同时间的批次，以此为根据去重
                ...
                if content:  #有的价格历史会显示‘数据还在完善中’，从这个url发过去返回为空
            ...
            with open('nongfu.txt', 'a') as f:
                f.write(json.dumps(result, ensure_ascii = False)+'\n')
            print ('done')


def get_content_and_check_ok(url, data):
    content = process._downloader.downloader_wrapper(url,
                batch_id,
                gap,
                method = 'post',
...
# url = 'http://datacenter.cngrain.com/amline/PriceMarkXml.aspx?MarketId=%E6%9B%B9%E6%B5%B7%E6%B6%9B&TId=97&GId=h971&PTId=111'
# a = DownloadWrapper(None, headers,'123')
# a.downloader_wrapper(url , '123',1)